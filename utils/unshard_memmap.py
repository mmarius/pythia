...
def unshard(
    ...
    base_filename = os.path.basename(input_file)[
        :-19
    ]  # remove 00000-of-xxxxx.bin suffix from shard 0's filename

    # check size of non-final shard
    shard_filename = shard_filename = (
        os.path.join(input_dir, base_filename) + f"-00000-of-{(num_shards - 1):05}.bin"
    )
    print("first shard filename", shard_filename)
    ...
    shard_filename = (
        ...
        + f"-{(num_shards - 1):05}-of-{(num_shards - 1):05}.bin"
    )
    print("final shard filename", shard_filename)
    shard_memmap = np.memmap(shard_filename, mode="r", order="C")
    ...
    full_idx_map = np.memmap(
        os.path.join(output_dir, base_filename) + ".bin",
